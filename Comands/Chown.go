...
func DataChown(context []string, partition Structs.Partition, pth string) {
	rBoolean := false
	user := ""
	path := ""
	...
	if path == "" || user == "" {
		Error("CHMOD", "Se necesitan parametros obligatorios para crear un directorio")
		return
	}
	tmp := GetPath(path)
	chown(tmp, rBoolean, partition, pth, user)
}

func chown(path []string, r bool, partition Structs.Partition, pth string, u string) {

	gid, uid := getUser(u, Logged.Id)
	copyPath := path
	...
	if err_ != nil {
		Error("CHOWN", "Error al leer el archivo")
		return
	}
	...
	if len(path) == 0 {
		Error("CHOWN", "No se ha brindado un path valido")
		return
	}
	...
	for v := 0; v < len(path)-1; v++ {
					...
					for j := 0; j < 4; j++ {
						...
						if Compare(nameFolder, path[v]) {
							...
							if inode.I_uid != int64(Logged.Uid) {
								Error("CHOWN", "No tiene permisos para crear carpetas en este directorio")
								return
							}
		...
		if !fnd {
			...
			Error("CHOWN", "No se pudo crear el directorio "+address+", no existen directorios")
			return
		}
	}
			...
			if i < 16 {
				...
				if folder.B_content[2].B_inodo != -1 {
					...
					if Compare(nameAux1, fileToFound) {
						...
						if r {
							for w := 0; w < 16; w++ {
								if inodetmp.I_block[w] != -1 {
									...
									recurChown(control, partition, pth, gid, uid)
								} else {
									break
								}
							}
						...
						Message("CHOWN", "Se ha cambiado el propietario de la carpeta "+ruta)
						break
					}
				}
...
func getUser(u string, id string) (string, string) {
	var path string
	partition := GetMount("LOGIN", id, &path)
	if string(partition.Part_status) == "0" {
		Error("CHOWN", "No se encontro la partición montada con el id: "+id)
		return "", ""
	}
	...
	for i := 0; i < len(vctr)-1; i++ {
		...
		if line[2] == 'U' || line[2] == 'u' {
			...
			if Compare(in[3], u) && in[0] != "0" {
				...
				if !exists {
					Error("CHOWN", "No se encontre el grupo \""+in[2]+"\".")
					return "", ""
				}
				return idGroup, in[0]
			}
	...
	Error("CHOWN", "No se encontró el usuario "+u)
	return "", ""
}

func recurChown(control int64, partition Structs.Partition, pth string, gid string, uid string) {
	spr := Structs.NewSuperBlock()
	inode := Structs.NewInodos()

	file, err := os.Open(strings.ReplaceAll(pth, "\"", ""))
	if err != nil {
		Error("CHMOD", "No se ha encontrado el disco")
		return
	}
	...
	err_ := binary.Read(buffer, binary.BigEndian, &spr)
	if err_ != nil {
		Error("CHMOD", "Error al leer el archivo")
		return
	}
		...
		if strings.Contains(nameAux1, ".") {
			...
			for w := 0; w < 16; w++ {
				if inode.I_block[w] != -1 {
					...
					recurChown(controlInt, partition, pth, gid, uid)
				} else {
					break
				}
			}
