	...
	for i := 0; i < len(context); i++ {
		...
		if Compare(tk[0], "id") {
			...
			if Compare(tk[1], "full") {
				...
				Error("MKFS", "EL comando type debe tener calores especificos")
				return
			}
			...
			if Compare(tk[1], "2fs") || Compare(tk[1], "3fs") {
				...
				Error("MKFS", "El comando fs debe tener valores especificos")
			}
		}
	}
	if id == "" {
...
func ext3(spr Structs.SuperBlock, p Structs.Partition, n int64, path string) {
