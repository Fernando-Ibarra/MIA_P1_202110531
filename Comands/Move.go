...
func DataMove(context []string, partition Structs.Partition, pth string) {
	destino := ""
	path := ""
	...
	move(tmp, tmp2, partition, pth)
}

func move(path []string, dest []string, partition Structs.Partition, pth string) {
	bi, nameFolderGet := getBi(path, partition, pth)
	copyPath := dest
	...
	if err != nil {
		Error("MOVE", "No se ha encontrado el disco")
		return
	}
	...
	if err_ != nil {
		Error("MOVE", "Error al leer el archivo")
		return
	}
	...
	if len(dest) == 0 {
		Error("MOVE", "No se ha brindado un path valido")
		return
	}
	...
	for v := 0; v < len(dest); v++ {
					...
					for j := 0; j < 4; j++ {
						...
						if Compare(nameFolder, dest[v]) {
							...
							if inode.I_uid != int64(Logged.Uid) {
								Error("MOVE", "No tiene permisos para crear carpetas en este directorio")
								return
							}
		...
		if !fnd {
			...
			Error("MOVE", "No se pudo crear el directorio "+address+", no existen directorios")
			return
		}
	}
			...
			if i < 16 {
				...
				for j := 0; j < 4; j++ {
					if folder.B_content[j].B_inodo == -1 {
						...
						Message("MOVE", "Se ha movido "+address2+" a "+address)
						break
					}
				}
			}
		...
		for i := 0; i < 16; i++ {
			if inode.I_block[i] == -1 {
				if i < 16 {
					...
					if past == -1 {
						Error("MOVE", "No se ha podido crear el directorio, el sistema de archivos ha alcanzado su maxima capacidad")
						return
					}
...
func getBi(path []string, partition Structs.Partition, pth string) (int64, string) {
	spr := Structs.NewSuperBlock()
	inode := Structs.NewInodos()
	folder := Structs.NewDirectoriesBlocks()
