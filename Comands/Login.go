...
func DataUserLogin(context []string) bool {
	id := ""
	user := ""
	pass := ""
	...
	return activeSession(user, pass, id)
}

func activeSession(u string, p string, id string) bool {
	var path string
	partition := GetMount("LOGIN", id, &path)
	if string(partition.Part_status) == "0" {
		Error("LOGIN", "No se encontro la partición montada con el id: "+id)
		return false
	}
...
func LogOut() bool {
	Message("LOGOUT", "¡ADIOS "+Logged.User+" , espero volver a verte!")
	Logged = ActiveUser{}
	return false
}
