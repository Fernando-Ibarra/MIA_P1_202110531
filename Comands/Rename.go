...
func DataRename(context []string, partition Structs.Partition, pth string) {
	name := ""
	path := ""
	...
	if path == "" || name == "" {
		Error("RENAME", "Se necesitan parametros obligatorios para crear un directorio")
		return
	}
	tmp := GetPath(path)
	rename(tmp, partition, pth, name)
}

func rename(path []string, partition Structs.Partition, pth string, name string) {
	copyPath := path
	...
	if err != nil {
		Error("RENAME", "No se ha encontrado el disco")
		return
	}
	...
	if err_ != nil {
		Error("CAT", "Error al leer el archivo")
		return
	}
	...
	if err_ != nil {
		Error("RENAME", "Error al leer el archivo")
		return
	}

	var fileToFound string
	if len(path) == 0 {
		Error("RENAME", "No se ha brindado un path valido")
		return
	}
	...
	for v := 0; v < len(path)-1; v++ {
					...
					for j := 0; j < 4; j++ {
						...
						if Compare(nameFolder, path[v]) {
							...
							if inode.I_uid != int64(Logged.Uid) {
								Error("RENAME", "No tiene permisos para crear carpetas en este directorio")
								return
							}
		...
		if !fnd {
			...
			Error("RENAME", "No se pudo crear el directorio "+address+", no existen directorios")
			return
		}
	}
			...
			if i < 16 {
				...
				if folder.B_content[2].B_inodo != -1 {
					...
					if Compare(nameAux1, fileToFound) {
						...
						ruta += "/" + name
						Message("RENAME", "Se ha actualiza la ruta: "+ruta)
						break
					}
				}
