...
func DataCat(context []string, partition Structs.Partition, pth string) {
	...
	if len(filesPath) == 0 {
		Error("CAT", "Se necesitan parametros obligatorios ")
		return
	}

	for i := 0; i < len(filesPath); i++ {
		tmp := GetPath(filesPath[i])
		content := cat(tmp, partition, pth)
		Message("CAT", "Contenido: "+content)
	}
}

func cat(path []string, partition Structs.Partition, pth string) string {
	...
	if err != nil {
		Error("CAT", "No se ha encontrado el disco")
		return ""
	}
	...
	if len(path) == 0 {
		Error("CAT", "No se ha brindado un path valido")
		return ""
	}
	...
	for v := 0; v < len(path)-1; v++ {
					...
					for j := 0; j < 4; j++ {
						...
						if Compare(nameFolder, path[v]) {
							...
							if inode.I_uid != int64(Logged.Uid) {
								Error("CAT", "No tiene permisos para crear carpetas en este directorio")
								return ""
							}
		...
		if !fnd {
			...
			Error("CAT", "No se pudo crear el directorio "+address+", no existen directorios")
			return ""
		}
	}
