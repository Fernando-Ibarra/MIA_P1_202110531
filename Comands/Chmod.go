...
func DataChmod(context []string, partition Structs.Partition, pth string) {
	rBoolean := false
	ugo := ""
	path := ""
	...
	tmp := GetPath(path)
	chmod(tmp, rBoolean, partition, pth, ugo)
}

func chmod(path []string, r bool, partition Structs.Partition, pth string, u string) {
	if !Compare(Logged.User, "root") {
		Error("CHMOD", "Solo el usuario \"root\" puede acceder a estos comandos")
		return
	}
	...
	if len(path) == 0 {
		Error("CHMOD", "No se ha brindado un path valido")
		return
	}
	...
	for v := 0; v < len(path)-1; v++ {
					...
					for j := 0; j < 4; j++ {
						...
						if Compare(nameFolder, path[v]) {
							...
							if inode.I_uid != int64(Logged.Uid) {
								Error("CHMOD", "No tiene permisos para crear carpetas en este directorio")
								return
							}
		...
		if !fnd {
			...
			Error("CHMOD", "No se pudo crear el directorio "+address+", no existen directorios")
			return
		}
	}
			...
			if i < 16 {
				...
				if folder.B_content[2].B_inodo != -1 {
					...
					if Compare(nameAux1, fileToFound) {
						...
						if r {
							for w := 0; w < 16; w++ {
								if inodetmp.I_block[w] != -1 {
									...
									recurChmod(control, partition, pth, ugo)
								} else {
									break
								}
							}
						...
						Message("CHMOD", "Se ha cambiado los permisos de la carpeta "+ruta)
						break
					}
				}
...
func recurChmod(control int64, partition Structs.Partition, pth string, ugo int) {
	spr := Structs.NewSuperBlock()
	inode := Structs.NewInodos()
		...
		if strings.Contains(nameAux1, ".") {
			...
			for w := 0; w < 16; w++ {
				if inode.I_block[w] != -1 {
					...
					recurChmod(controlInt, partition, pth, ugo)
				} else {
					break
				}
			}
